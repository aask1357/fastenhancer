...
def process_single_line(args):
    ...
    try:
        audio = librosa.load(
            worker_data_dir / file, sr=worker_fs, mono=True,
...
def get_args():
    ...
    parser.add_argument(
        "-d", "--data-dir",
        type=Path,
        default="/home/shahn/Datasets/DNS-Challenge/16khz/clean_wo_german_w_singing",
        help="Data directory."
    )
    parser.add_argument(
        "-s", "--shar-dir",
...
def main():
    ...
    try:
        ...
        files = [f for f in args.data_dir.rglob(f"*.{args.extension}", recurse_symlinks=True)]
    except Exception as e:
        # For older Python versions.
        files = list(rglob_with_symlinks(args.data_dir, args.extension))
    if args.debug:
        args.num_jobs = 1
        files = files[:10]
    print("✅")
