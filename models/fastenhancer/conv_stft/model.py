...
class ConvSTFT(nn.Module):
    def __init__(
        ...
        self.n_freq = n_fft // 2 + 1 if not discard_last_freq_bin else n_fft // 2
        self.normalized = normalized
        self.discard_last_freq_bin = discard_last_freq_bin
        self.output_padding = (n_fft - hop_size) % 2
        ...
        # Get STFT weight
        k = torch.arange(self.n_freq, **factory_kwargs).view(-1, 1, 1)
        n = torch.arange(n_fft, **factory_kwargs).view(1, 1, -1)
        idx = -2 * math.pi / self.n_fft * k * n
        ft_weight = torch.cat([torch.cos(idx), torch.sin(idx)], dim=0)
        if normalized:
            ft_weight /= math.sqrt(self.n_fft)
        self.register_buffer("weight", window * ft_weight, persistent=False)
        self.weight: Tensor
        ...
        window_istft = 1 / win_sq_sum
        if not normalized:
            window_istft /= self.n_fft
    ...
    def forward_fft(self, x: Tensor, cache: Tensor) -> tp.Tuple[Tensor, Tensor]:
        '''x: [B=1, hop_size]
    ...
    def forward(self, x: Tensor, cache: Tensor) -> tp.Tuple[Tensor, Tensor]:
        '''x: [B=1, hop_size]
        cache: [B=1, n_fft-hop_size]
        output: [B, n_fft//2(+1), T=1, 2]
        '''
        x = torch.cat([cache, x], dim=1)  # [B, n_fft]
        cache = x[:, -self.cache_len:]    # [B, n_fft-hop_size]
        x = x.unsqueeze(1)                # [B, 1, n_fft]
        x = F.conv1d(x, self.weight, None, stride=self.hop_size)
        B, C, T = x.shape
        x = x.view(B, 2, C//2, T).permute(0, 2, 3, 1)   # [B, Freq, T, 2]
        return x, cache

    def inverse(self, x: Tensor, cache: Tensor) -> tp.Tuple[Tensor, Tensor]:
        '''input:
            x: [B, N//2(+1), T=1, 2]
            cache: [B, N-H]
        output:
        ...
        x_0 = x[:, 0:1, 0, 0]
        if not self.discard_last_freq_bin:
            x_last = x[:, -1:, 0, 0]
        x = x.permute(0, 3, 1, 2).reshape(-1, self.n_freq * 2, 1)   # [B, 2*Freq, 1]
        x = F.conv_transpose1d(x, self.weight, bias=None, stride=self.hop_size) # [B, 1, n_fft]
        if self.discard_last_freq_bin:
            correction = x_0.view(-1, 1, 1) * self.window.view(1, 1, -1)    # [B, 1, n_fft]
            x = 2 * x - correction
            # x_last = 0
            # x = x.view(-1, self.n_fft//2, 2)
            # correction = torch.stack([x_0 + x_last, x_0 - x_last], dim=2)       # [B, 1, 2]
            # correction = self.window.view(1, self.n_fft//2, 2) * correction     # [B, n_fft//2, 2]
            # x = 2 * x - correction
        else:
            x = x.view(-1, self.n_fft//2, 2)
            correction = torch.stack([x_0 + x_last, x_0 - x_last], dim=2)       # [B, 1, 2]
        ...
        x[:, :self.cache_len] += cache
        out = x[:, :-self.cache_len]      # [B, H*T]
...
class ONNXModel(nn.Module):
    ...
    def forward(
        ...
        *args
        ...
        """ input/output: [B, n_fft//2, T_spec, 2]"""
        # Compress
...
def test_conv_stft():
    ...
    T = H * 5
    discard = True
    stft = ConvSTFT(N, H, N, win_type="hann", discard_last_freq_bin=discard, dtype=torch.float64)
    x = torch.randn(1, T, dtype=torch.float64)
    x_ = x
    if discard:
        y = x.stft(n_fft=N, hop_length=H, win_length=N, window=stft.window, return_complex=True)
        print(y.shape)
        y[:, -1, :] = 0.
        x_ = y.istft(n_fft=N, hop_length=H, win_length=N, window=stft.window, return_complex=False)

    cache_in, cache_out = stft.initialize_cache(x)

    z_list = []
    for t in tqdm(range(0, T // H * H, H)):
        # cc = cache_in
        y, cache_in = stft(x[:, t:t+H], cache_in)
        # y, cache_in = stft.forward2(x[:, t:t+H], cache_in)
        # breakpoint()
        z, cache_out = stft.inverse(y, cache_out)
        z_list.append(z)
    z = torch.cat(z_list, dim=1)
    x = torch.cat([x.new_zeros(1, stft.cache_len), x_], dim=1)[:, :z.size(1)]
    plt.plot(x.squeeze(0), label="x")
    plt.plot(z.squeeze(0) - x.squeeze(0), label="diff")
